...
server.onRequest = function(req, res) {
	...
	/**
	 * 20-07-2020
	 * Discovered problematic request in logs (/safe=true/ instead of /?safe=true(/))
	 */
	if (req.prerender.url.match(/\/safe=true\//)) {
		
		const newUrl = req.prerender.url.replace(/\/safe=true\//, '/?safe=true');
		console.error(`Problematic request, rewriting ${req.prerender.url} to ${newUrl}`);
	...
	.then(() => {
		...
		req.prerender.content = req.prerender.tab.prerender.content.replace(/\<img src/g, '<img loading="lazy" src'); // SEO lighthouse fix;
